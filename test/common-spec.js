...
describe('common', function(){
    ...
    describe("closestPosition", function() {
        it("should work", function() {
            expect(common.closestPosition({x: 0, y: 0}, [])).to.equal(undefined);
            expect(common.closestPosition({x: 0, y: 0}, [{x: 0, y: 0}])).to.deep.equal({distance: 0, position: { x: 0, y: 0}});
            expect(common.closestPosition({x: 0, y: 0}, [{x: 0, y: 0}, {x: 1, y: 1}])).to.deep.equal({distance: 0, position: {x: 0, y: 0}});
            expect(common.closestPosition({x: 0, y: 0}, [{x: 2, y: 1}, {x: 1, y: 1}])).to.deep.equal({distance: 2, position: {x: 1, y: 1}});
        });
    });
